...
export default connect(mapStateToProps, mapDispatchToProps)(function Statistic(props) {
    ...
    const { allUsers, allPackagesByUser, allPackages, getAllUsers, getAllPackages, getAllPackagesByUsers } = props;
    const [usersAgesStatistic, setUsersAgesStatistic] = useState({});

    useEffect(async () => {
        await getAllPackagesByUsers()
        await getAllPackages()
        await getAllUsers()
        await JSON.parse(localStorage.getItem('allUsers'))?.users?.map((item) => {
    ...
    return (
        <>
            <div className="buttons-for-navigat row">
                ...
                <div className="btn-nabigate col" onClick={() => navigateToPages('packages')}>Create new Package</div>
