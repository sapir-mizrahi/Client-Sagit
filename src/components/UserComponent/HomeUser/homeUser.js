...
export default connect(mapStateToProps, mapDispatchToProps)(function HomeUser(props) {
    const { packagesByUser, getCourseDetailsByCourseId,
        courseDetails, setCurrentVideoLesson, getPackagesByCurrentUser } = props;
    ...
    const getAllCoursesByCurrentUser = async () => {
        let tempArrCategory = []
        let tempArrCourses = []
        // setAa((JSON.parse(localStorage.getItem('coursesByUser'))).packages)
        // console.log("aa", aa);
        await Array.isArray((JSON.parse(localStorage.getItem('coursesByUser'))).packages) && (JSON.parse(localStorage.getItem('coursesByUser'))).packages.length && (JSON.parse(localStorage.getItem('coursesByUser'))).packages?.map(async (itemPackage) => {
